...
const PRODUCTS = [
    {name: 'The Annie', url: 'https://www.tecovas.com/products/the-annie', filter_type: 'product_id', filter_value: '1496563'},
    {name: 'The Doc', url: 'https://www.tecovas.com/products/the-doc', filter_type: 'product_id', filter_value: '1497168'},
    {name: 'The Jamie', url: 'https://www.tecovas.com/products/the-jamie', filter_type: 'product_id', filter_value: '2632633'},
];
...
function getProductId() {
    if (typeof window === 'undefined') {
        console.log('getProductId called in non-browser environment');
    }
    const currentProduct = PRODUCTS.find(product => 
        window?.location?.href?.includes(product.url)
    );
    if (!currentProduct?.filter_value) {
        console.error('Product ID not found');
        return null;
    }
    return currentProduct.filter_value;
}
...
async function generateSuggestedQueries({query, usePreGeneratedSuggestions=usePreGeneratedContent, count=3, productId=getProductId()}) {
    console.log('Generating suggested queries for:', query);
    const productName = getProductName(productId);
    let suggestedQueries;
    if (typeof window === 'undefined') {
        console.log('- productName:', productName);
        console.log('- productId:', productId);
        console.log('- usePreGeneratedSuggestions:', usePreGeneratedSuggestions);
        console.log('- initialSuggestedQueriesDisplayed:', initialSuggestedQueriesDisplayed);
    }
    try {
        if (usePreGeneratedSuggestions && preGeneratedSuggestions[productId] && !initialSuggestedQueriesDisplayed) {
            if (typeof window !== 'undefined') {
                suggestedQueries = preGeneratedSuggestions[productId];
                displaySuggestedQueries(suggestedQueries);
            }
        } else {
            suggestedQueries = await generateSuggestedQueriesCall({query, productName, productId});
...
async function generateSuggestedQueriesCall({query, productName, count=3, productId=getProductId()}) {
    console.log('Generating suggested queries for:', query);
    console.log('productName:', productName);
    console.log('count:', count);
    // get the trackingIDs of the latest search results
    let chunkTrackingIds = latestSearchResultChunks.map(chunk => chunk.tracking_id);
    if (!chunkTrackingIds || chunkTrackingIds.length === 0) {
        ...
        chunkTrackingIds = searchResults.chunks.map(chunk => chunk.chunk.tracking_id);
        if (typeof window === 'undefined') {
            console.log('Chunk tracking IDs:', chunkTrackingIds);
        }
    }
        ...
        GM_xmlhttpRequest({
            ...
            onload: function(response) {
                try {
                    ...
                    console.log('(generateSuggestedQueriesCall) data.queries:', data.queries);
...
async function getSummaryOfReviews({usePreGeneratedReviewSummary=usePreGeneratedContent, productId=getProductId()}) {
    let summary;
    try {
        if (usePreGeneratedReviewSummary) {
            if (preGeneratedSummary && preGeneratedSummary[productId]) {
                ...
                throw new Error('Pre-generated summary not found for product ID: ' + getProductId);
            }
        } else {
...
async function createTopicAndMessage(userMessage, productId=getProductId()) {
    ...
    try {
        ...
        if (typeof window !== 'undefined') {
            addMessageToChatArea('Error: Unable to process your request.', false, true);
        } else {
...
async function createMessageCall(topicId, messageContent, productId=getProductId()) {
...
function createMessageCallServer(requestBody) {
    console.log('createMessageCallServer called');
...
function createMessageCallBrowser(requestBody, messageContent) {
    return new Promise((resolve, reject) => {
        ...
        GM_xmlhttpRequest({
            ...
            data: JSON.stringify(requestBody),
            responseType: 'stream',
            onloadstart: handleStreamResponse(externalTimeoutId, messageContent, resolve, reject),
            onerror: handleCreateMessageError(externalTimeoutId, reject)
        });
    });
}

function handleStreamResponse(externalTimeoutId, messageContent, resolve, reject) {
    ...
    function processBuffer() {
        ...
        if ((separatorIndex = buffer.indexOf('||[{"id"')) !== -1) {
            ...
            const [bufferBeforeSeparator, bufferAfterSeparator] = buffer.split('||[{"id"', 2);
...
function displaySearchResults(results, query) {
    ...
    if (previousResultsList) {
        ...
        const updateButtonText = (isShowing) => {
            ...
            queryButton.textContent = `${action} reviews related to: "${previousResultsList.getAttribute('data-query')}"`;
        };
    ...
    const searchResultsTitle = searchResultsDiv.querySelector('#search-results-title');
    ...
    results.forEach(result => {
        if (result.chunk_html) {
            ...
            if (headingText) {
                ...
                if (contentText.startsWith(comparisonHeadingText) || 
...
function generateInitialSuggestions({usePreGeneratedSuggestions=usePreGeneratedContent, productId=getProductId()}) {
    let initialSuggestions;
    if (usePreGeneratedSuggestions && preGeneratedSuggestions[productId]) {
        console.log('Using pre-generated suggestions for product ID:', productId);
        ...
        const productQuery = `${getProductName(productId)} ${getBreadcrumbsContent() || 
                            title || 
                            '(this product)'}`;
        initialSuggestions = generateSuggestedQueries({query: productQuery, usePreGeneratedSuggestions: usePreGeneratedSuggestions, productId: productId});
    }
...
function addMessageToChatArea(message, isUser = false, isError = false) {
    if (typeof window === 'undefined') {
        console.log('addMessageToChatArea called in non-browser environment');
        return;
    }
    ...
    if (!isUser && message.trim() === '...') {
        ...
        messageElement.className = isUser ? 'user-message' : 
...
function updateAIResponse(response) {
    if (typeof window === 'undefined') {
        console.log('updateAIResponse called in non-browser environment');
        return;
    }
    const chatArea = document.getElementById('trieve-chat-area');
...
function adjustChatAreaHeight() {
    if (typeof window === 'undefined') {
        console.log('adjustChatAreaHeight called in non-browser environment');
        return;
    }
    const chatArea = document.getElementById('trieve-chat-area');
...
function displaySuggestedQueries(queries, count = 10) {
    console.log('Displaying suggested queries');
    if (typeof window === 'undefined') {
        console.log('Suggested queries:', queries);
        return;
    }
    // Use a timeout to ensure the DOM has had time to update
    setTimeout(() => {
    ...
    queries.slice(0, count).forEach(query => {
        ...
        button.addEventListener('click', () => {
            if (!button.disabled) {
                ...
                if (typeof window === 'undefined') {
                    console.log('Suggested query clicked in non-browser environment');
                    return;
                }
                const input = document.getElementById('trieve-input');
                input.value = query;
...
function insertTrieveComponent() {
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        chatArea.style.height = `${Math.max(minHeight, Math.min(newHeight, maxHeight))}px`;
    });
    ...
    function insertComponent(targetElement) {
        ...
        console.log('Trieve component inserted successfully');
    }
    ...
    generateInitialSuggestions({usePreGeneratedSuggestions: usePreGeneratedContent});
    ...
    sendButton.addEventListener('click', () => handleUserInput(input.value.trim()));
}
